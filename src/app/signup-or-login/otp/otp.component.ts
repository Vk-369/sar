...
export class OtpComponent {
  ...
  constructor(private _router: Router, private _route: ActivatedRoute, private _sarService: SarServiceService){
  }
  ...
  singleDigit(control: string){
    if(this.otpForm.get(control)?.value.length >= 2){
      ...
      this.otpForm.get(control)?.setValue(this.otpForm.get(control)?.value.slice(0,1))
